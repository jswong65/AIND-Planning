...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # if the specified level does not exist, initiate the level
        while len(self.a_levels) < (level + 1):
            self.a_levels.append(set())

        #if action_node.prenodes.issubset(self.s_levels[level]):
            #    self.a_levels[level].add(action_node)

        # add PgNode_a to current a_level, and connect it with a PgNode_s
    ...
    def add_literal_level(self, level):
        ...
        # if the specified level does not exist, initiate the level
        while len(self.s_levels) < (level + 1):
            self.s_levels.append(set())
    
        # add PgNode_s to current s_level, and connect it with a PgNode_a
        ...
        """    
        for a_node in self.a_levels[level - 1]:
            s_nodes_set = s_nodes_set.union(a_node.effnodes)
        self.s_levels[level] = s_nodes_set
        """
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (node_a1.action.effect_add == node_a2.action.effect_rem or
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # four possible cases for negation of a precondition and an effect.
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #check whether one of the parent node is mutex with each other
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # Check whether every pair of actions is mutex
        for pre_a1 in node_s1.parents:
            for pre_a2 in node_s2.parents:
                if not pre_a1.is_mutex(pre_a2):
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        '''
        level_sum = 0
        goal_set = set() # a data structure to prevent add s_nodes that has same states.
        explored = set()
        problem_goal = set(self.problem.goal)
        # TODO implement
        # for each goal in the problem, determine the level cost, then add them together
        
        # loop through each s_node in every s_levels to find s_nodes in the lowest levels
        # that has the state in goal state.  
        
        
        for level in range(len(self.s_levels)):
            for s_node in self.s_levels[level]:
                if s_node.is_pos and (s_node.symbol not in goal_set) and \
                     (s_node.symbol in problem_goal):
                     level_sum += level
                     goal_set.add(s_node.symbol)


                     if len(goal_set) == len(problem_goal):
                        # identify all states that satisfy goals
                        return level_sum
